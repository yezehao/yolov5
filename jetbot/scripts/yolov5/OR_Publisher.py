...
def run(
        ...
        source= 1,  #Use the webcam
        data=ROOT / 'data/coco128.yaml',  # dataset.yaml path
        imgsz=(240, 135),  # inference size (height, width)
    ...
    view_img = 0  #Do not show the image window
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                # Publish 'Object Recognized' when the duck is recognized
                for c in det[:, 5].unique():
                    n = (det[:, 5] == c).sum()  # detections per class
                    s += f"{n} {names[int(c)]}{'s' * (n > 1)}, "  # add to string
                    status = "Object Recognized"
                ...
                #Otherwise publish 'No Object Recognized'
                status = "No Object Recognized"
                pub.publish(status)
            # Stream results
            im0 = annotator.result()
            if view_img:  #The window will not be shown since the virw_img was set to 0 above
                if platform.system() == 'Linux' and p not in windows:
                    windows.append(p)
                    cv2.namedWindow('Object_Recognition_View', cv2.WINDOW_NORMAL | cv2.WINDOW_KEEPRATIO)  # allow window resize (Linux)
